<!-- pride season

================================================== -->
<section class="pt-8 pb-8 pt-md-11 pb-md-11 bg-white">
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-12 col-lg-6 text-center">

                <!-- Heading -->
                <h2 class="display-6 font-weight-bold text-dark">
                    National Coming Out Day
                </h2>
                <h2 class="display-3 font-weight-bold text-dark">
                    Care Package
                </h2>
                <p class="text-muted lead mb-6 pb-0">
                    <span class="text-dark">Care packages will be delivered in time for coming out day.</span>
                </p>

                <a class="btn btn-dark btn-pill" href="https://freemomhugs.org/applications/coming-out/">
                    <span style="background: linear-gradient(to right, #70d6ff, #ff70a6, #ff9770); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">Apply Now</span>
                </a>

            </div>

        </div> <!-- / .row -->
    </div>
</section>
<?php
